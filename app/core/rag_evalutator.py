...
class RAGEvaluator:
    ...
    def __init__(self, cfg: DictConfig):
        ...
        logger.info("Initialize RAG Evaluator")
        self.cfg = cfg

        eval_llm_model = self.cfg.ragas.llm_model or self.cfg.model.llm_model
        eval_llm_temperature = (
            self.cfg.ragas.llm_temperature
            if self.cfg.ragas.llm_temperature is not None
            else self.cfg.model.llm_temperature
        )
        eval_embedding_model = self.cfg.ragas.embedding_model or self.cfg.model.embedding_model

        self.llm = ChatOpenAI(
            model = eval_llm_model,
            temperature = eval_llm_temperature,
            openai_api_key=self.cfg.openai.api_key,
        )
